...
void odomCallback(const nav_msgs::Odometry::ConstPtr &odom_msg) {
    ...
    // 计算车辆当前朝向与点的夹角
    std::array<double, 2> target_point = {path.poses[target_index].pose.position.x,
    ...
    ROS_INFO("[ Pure Pursuit param ] : alpha: %.2f, delta: %.2f , Lf: %.2f", alpha, delta, Lf);
    ...
    for (int i = closest_index; i + 1 < path.poses.size(); ++i) {
        ...
        ROS_INFO("[ Pure Pursuit param ] : delta_distance: %.2f", delta_distance);
 ...
 */
void loadPath() {
    std::string input_bag_filename = "/home/whale/data/odom.bag";
