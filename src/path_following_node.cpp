...
void odomCallback(const nav_msgs::Odometry::ConstPtr &odom_msg)
{
    ...
    {
        double temp_lf = Lf;
        double dx = path.poses[i].pose.position.x - path.poses[closest_index].pose.position.x;
        double dy = path.poses[i].pose.position.y - path.poses[closest_index].pose.position.y;
    ...
    // 计算车辆当前朝向与点的夹角
    std::array<double, 2> target_point = {path.poses[target_index].pose.position.x, path.poses[target_index].pose.position.y};
    std::array<double, 2> current_point = {current_pose.pose.position.x, current_pose.pose.position.y};
    ...
    delta = std::max(std::min(delta, Constants::max_steer), -Constants::max_steer);
    geometry_msgs::Twist cmd;
    cmd.angular.z = delta;
    cmd.linear.x = Constants::kv * last_v + (1 - Constants::kv) * Constants::max_speed;

    double distance_to_goal = std::hypot(current_pose.pose.position.x - path.poses.back().pose.position.x,
    ...
    ROS_INFO("closest_index: %d, teb_target_index: %d", closest_index, teb_target_index);
    ROS_INFO("delta_distance: %.2f", delta_distance);
}
...
void loadPath()
{
    std::string input_bag_filename = "/home/wangyu/path_follow/src/path_following/src/odom_data.bag";
    ...
    for (rosbag::MessageInstance const m : view)
    {
        ...
        if (odom_msg != nullptr)
        {
            ...
            double distance = std::hypot(pose_stamped.pose.position.x - last_added_point.x,
